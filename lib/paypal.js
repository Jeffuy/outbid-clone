...
function safeDiagnostic(value, maxLength) {
  if (typeof value !== "string") return undefined;
  return value.replace(/[\u0000-\u001f\u007f]/g, " ").trim().slice(0, maxLength) || undefined;
}

export class PayPalError extends Error {
  constructor(message, status, { paypalName, issues, debugId } = {}) {
    super(safeDiagnostic(message, 500) || "PayPal request failed");
    this.name = "PayPalError";
    this.status = status;
    this.paypalName = safeDiagnostic(paypalName, 100);
    this.issues = Array.isArray(issues)
      ? issues.slice(0, 20).map((detail) => ({
        issue: safeDiagnostic(detail?.issue, 100),
        field: safeDiagnostic(detail?.field, 250),
        description: safeDiagnostic(detail?.description, 500),
      })).filter((detail) => detail.issue || detail.field || detail.description)
      : [];
    this.debugId = safeDiagnostic(debugId, 100);
  }
}
...
async function paypalFetch(path, options = {}) {
  ...
  if (!response.ok) {
    throw new PayPalError(data?.message || "PayPal request failed", response.status, {
